// vkrelay2 VBO canary.
//
// The on-screen artifact for the memory-visibility spine: a native Vulkan program that creates an
// xcb window, then drives the whole path -- allocate a HOST_VISIBLE|HOST_COHERENT allocation,
// create + bind a VERTEX buffer, **persistently map it and write the triangle once (NO
// vkFlushMappedMemoryRanges on the hot path)**, build a pipeline whose vertex input matches the
// buffer, and per frame: acquire -> record(begin / bind pipeline / set viewport / set scissor /
// bind vertex buffers / draw(3) / end) -> submit -> wait -> present. Because the canary never
// flushes explicitly, the colored triangle on screen is specifically proof of the ICD's
// coherent-flush-at-submit rule: the mapped bytes reach vertex fetch only because
// vkQueueSubmit sweeps + ships them.
//
...
void log(const char* msg) {
    std::fprintf(stderr, "vkrelay2-vbo: %s\n", msg);
}

bool check(VkResult r, const char* what) {
    if (r != VK_SUCCESS && r != VK_SUBOPTIMAL_KHR) {
        std::fprintf(stderr, "vkrelay2-vbo: %s failed (VkResult %d)\n", what, static_cast<int>(r));
        return false;
    }
    return true;
}
...
int main() {
    ...
    app.pApplicationName = "vkrelay2-vbo";
    ...
    std::fprintf(stderr, "vkrelay2-vbo: device '%s'\n", props.deviceName);
    ...
    {
        // The vertex buffer: create, pick a HOST_VISIBLE|HOST_COHERENT type allowed by its
        // requirements, allocate, bind, then map ONCE and write the triangle. The map stays live
        // and there is NO explicit flush -- the coherent-flush-at-submit sweep ships these bytes.
        VkBufferCreateInfo bci{};
        bci.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        bci.size = sizeof(kVerts);
        bci.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
        bci.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
        if (!check(vkCreateBuffer(device, &bci, nullptr, &vbo), "vkCreateBuffer")) {
            goto teardown;
        }
        VkMemoryRequirements mreq{};
        vkGetBufferMemoryRequirements(device, vbo, &mreq);
        ...
        if (type_index == UINT32_MAX) {
            log("no HOST_VISIBLE|HOST_COHERENT memory type");
            goto teardown;
        }
        ...
        if (!check(vkAllocateMemory(device, &mai, nullptr, &vbo_mem), "vkAllocateMemory")) {
            goto teardown;
        }
        if (!check(vkBindBufferMemory(device, vbo, vbo_mem, 0), "vkBindBufferMemory")) {
            goto teardown;
        }
        void* mapped = nullptr;
        if (!check(vkMapMemory(device, vbo_mem, 0, VK_WHOLE_SIZE, 0, &mapped), "vkMapMemory") ||
            mapped == nullptr) {
            goto teardown;
        }
        std::memcpy(mapped, kVerts, sizeof(kVerts)); // persistent map, no explicit flush
        ...
        stages[1].pName = "main";
        // Vertex input matching the mapped buffer: one VERTEX-rate binding (stride 20), pos at 0,
        // color at 8.
        ...
        for (int frame = 0; frame < kFrames; ++frame) {
            ...
            if (acq == VK_ERROR_OUT_OF_DATE_KHR) {
                ...
                continue; // no resize path in this canary
            }
            ...
            if (pres == VK_SUCCESS || pres == VK_SUBOPTIMAL_KHR) {
                ++presented;
                std::fprintf(stderr, "vkrelay2-vbo: presented frame %d\n", frame);
            }
            sleep_ms(150);
        }
        rc = (presented >= 1) ? 0 : 1;
        std::fprintf(stderr, "vkrelay2-vbo: presented %d frame(s)\n", presented);
    }
